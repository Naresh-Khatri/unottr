...
import { api, onOverviewChanged } from "@/ipc/client";
import type {
  AiSettings, OverviewBullet, OverviewPayload, Person, Segment, Speaker, Task,
} from "@/ipc/types";
...
export function OverviewPanel({ recordingId, segments, speakers, ready, onSeek, onOpenSettings }: Props) {
  ...
  async function generate() {
    if (ai && !ai.consented) { setConsenting(true); return; }
    setError(null);
    setBusy(true);
    try {
  ...
  async function accept() {
    await api.setSetting("ai_consented", "1");
  ...
  if (ai && !ai.key_set)
    return (
      <Empty>
        <p>The overview is generated by {ai.model}, which needs an API key.</p>
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              {overview.error_kind === "auth" && (
                <Button size="sm" variant="outline" onClick={onOpenSettings}>Check the key</Button>
              )}
            </div>
          </>
        ) : (
          <>
            <p>No overview yet. Generating one sends this transcript to {ai?.model ?? "the model"}.</p>
 ...
 */
function Consent({ model, excerpt, onAccept, onCancel }: {
  model: string; excerpt: string; onAccept: () => void; onCancel: () => void;
}) {
  return (
    <div className="flex h-full flex-col gap-3 overflow-y-auto p-4 text-sm">
      <h3 className="font-medium">This sends the transcript to {model}</h3>
      <p className="text-muted-foreground">
        Text only — never the audio or the video. Nothing else in unottr talks to a network.
        This is what goes, verbatim:
      </p>
 ...
 */
function Generating({ startedAt, lines, onCancel }: {
  startedAt: number | null; lines: number; onCancel: () => void;
}) {
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex flex-col items-center gap-3 px-8 pt-10 pb-6 text-center text-sm text-muted-foreground">
        ...
        <div className="space-y-1">
          <p className="font-medium text-foreground">Reading the transcript</p>
          <p className="text-xs">
            {lines.toLocaleString()} lines · arrives all at once, usually under a minute
          </p>
        </div>
