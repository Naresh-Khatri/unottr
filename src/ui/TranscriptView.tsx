...
import {
  ArrowLeft, CaretDown, CaretUp, Export, MagnifyingGlass, PencilSimple, VideoCameraSlash,
...
export function TranscriptView({ id, onBack, initialMs = 0 }: {
  ...
  useEffect(() => {
    api.getRecording(id).then((d) => { setDetail(d); setSpeakers(d.speakers); });
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-4 p-4">
          <div className="flex flex-col gap-3">
            {detail.recording.available ? (
              <VideoPlayer path={detail.recording.path} currentMs={currentMs} onTimeUpdate={setCurrentMs} />
            ) : (
              <Card className="flex aspect-video items-center justify-center border-dashed">
                <CardContent className="flex flex-col items-center gap-2 text-center text-sm text-muted-foreground">
                  ...
                  Recording unavailable.<br />Transcript stays searchable.
                </CardContent>
              </Card>
            )}
            <Card>
